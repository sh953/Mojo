...
def function():
    ...
    if client_box_id is not None:
        ...
        if fetch_observing_devices_r.status_code == 200:
            ...
            for i in range(len(observing_devices_json)):
                ...
                if device_box_id == 19:
                    continue #we don't have coords for this one...
    ...
    apLocs = {}
    ap_loc_map = open('ap_coords.txt','r')
    for line in ap_loc_map.readlines():
        toks = line.split()
        apLocs[toks[1]] = (float(toks[2]), float(toks[3]))
    ...
    return ("""
        ...
        <div class='circle' style='left:""" + str(location[0] * pixel_to_feet) + "px; top:" +
        str(location[1] *pixel_to_feet) + """px'></div>
    """)
