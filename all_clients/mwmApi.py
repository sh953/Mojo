...
PATH_OBSERVING_DEVICES = "devices/clients/{boxId}/observingdevices"
...
class MwmApi:
    ...
    def get_observing_aps(self, boxId):
            response = self.request(PATH_OBSERVING_DEVICES.format(boxId=boxId))
            #print(response)
            if response and response.status_code == requests.codes.ok:
                return response.json()
            elif response:
                print('Could not fetch observing managed devices for boxId %d: %s'%(
                    boxId, response.status_code))
            else:
                print('Got no response for boxId %d'%boxId)
